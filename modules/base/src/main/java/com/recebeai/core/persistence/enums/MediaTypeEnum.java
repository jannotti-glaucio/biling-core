package tech.jannotti.billing.core.persistence.enums;

import org.apache.commons.lang.ArrayUtils;
import org.apache.commons.lang.StringUtils;

import tech.jannotti.billing.core.commons.exception.EnumNotFoundException;

import lombok.Getter;

public enum MediaTypeEnum {
    EMAIL("E"), WHATSAPP("W"), SMS("S");

    private @Getter String code;

    private MediaTypeEnum(String code) {
        this.code = code;
    }

    public static MediaTypeEnum valueOfCode(String code) {
        if (StringUtils.isBlank(code))
            return null;

        for (MediaTypeEnum value : values()) {
            if (value.code.equals(code))
                return value;
        }
        throw new EnumNotFoundException(code);
    }

    public static MediaTypeEnum[] valueOfArray(String[] names) {

        if (ArrayUtils.isEmpty(names))
            return null;

        MediaTypeEnum enums[] = new MediaTypeEnum[names.length];
        for (int i = 0; i < names.length; i++) {
            String name = names[i];
            enums[i] = valueOf(name);
        }

        return enums;
    }

}
